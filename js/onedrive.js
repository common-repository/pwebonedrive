...
var PWebOneDrive = {
    ...
    i18n: {
        ...
        button_file: "",
        emergency_exit: "",
        gallery_folder_select_warning: "",
        gallery_file_select_warning: "",
        folder_file_select_warning: ""
    },
    initialized: false,
    user: null,
    token: null,
    ...
    browseFolder: function (options)
    {
        ...
        WL.login({"scope": "wl.signin wl.skydrive wl.offline_access"}).then(
                function (response) {
                    if (that.options.debug) {
                        console.log("WL.login response:\n" + JSON.stringify(response));
                        if (that.options.debug == 1)
                            alert("Logged in successfully.");
                    }

                    //TODO message on first open: select a folder, not a file
                    ...
                    WL.fileDialog({
                        ...
                        select: 'multi'
                                ...
                                if (response.data.folders && response.data.folders.length) {
                                    ...
                                    for (var i = 0; i < folders.length; i++) {
                                        ...
                                        if (that.addToStore(folders[i].id)) {
                                            that.insertTag("onedrivefolder", folders[i].id);
                                            inserted++;
                                        }
                                    }
                                    if (inserted == 0) {
                                    ...
                                    alert(that.i18n.folder_file_select_warning);
                                    // reopen OneDrive browser
                                    that.browseFolder();
                                    return;
                                }
                            ...
                            function (response) {
                                if (that.options.debug) {
                                    console.log("browseFolder fileDialog error: " + response.error.code);
                                }
                                if (response.error.code == "invalid_cookie" || response.error.code == "request_token_expired") {
                                    that.resetCookie();
                                    if (that.autoreopen < 5)
                                        that.browseGallery();
                                } else {
                ...
                function (response) {
                    if (that.options.debug) {
                        console.log("browseFolder login error: " + response.error);
                    }
                    if (response.error == "invalid_cookie" || response.error == "access_denied") {
                        that.resetCookie();
                        if (that.autoreopen < 5)
                            ...
                            function (response) {
                                ...
                                if (response.data.folders && response.data.folders.length) {
                                    ...
                                    if (selected == 0) {
                                        ...
                                        alert(that.i18n.gallery_folder_select_warning);
                                        // reopen OneDrive browser
                                        that.browseGallery();
                                        return;
                                    } else if (inserted == 0) {
                                    ...
                                    // file selected instead of folder
                                    alert(that.i18n.gallery_file_select_warning);
                                    // reopen OneDrive browser
                                    that.browseGallery();
                                    return;
                                }
